...
class RailRobot(RobotModel):
    """
    A very simple robot with a single prismatic DOF
    """
    def __init__(self):
        self.L0 = 0.2
        self.L1 = 0.5
        self.link_offset=0.0
        self.T_0_1 = translation([0,0.0,self.L0])
    ...
    def getBaseFromToolTransform(self, joints):
        return (self.T_0_1 @ translation(np.array([joints[0],0,0])))

    def computeMGD(self, joints):
        return np.array([joints[0]])

    def analyticalMGI(self, target):
        return 1, np.array(target[0])

    def computeJacobian(self, joints):
        J = np.ones((1,1),dtype=np.double)
        return J

class PendulumRobot(RobotModel):
    """
    A very simple robot with a single revolute DOF
    """
    def __init__(self):
        self.L0 = 0.95
        self.L1 = 0.5
        self.link_offset=0.0
        self.T_0_1 = translation([0,0.1,self.L0])
        self.T_1_E = translation([self.L1,0,0])
    ...
    def getBaseFromToolTransform(self, joints):
        return (self.T_0_1 @ rot_y(joints[0]) @ self.T_1_E)
...
class RRRRobot(RobotModel):
    ...
    def computeJacobian(self, joints):
        ...
        J[:,0] = (self.T_0_1 @ d_rot_z(joints[0]) @ self.T_1_2 @
                  ...
                  self.T_3_E)[:3,3]
        J[:,1] = (self.T_0_1 @ rot_z(joints[0]) @ self.T_1_2 @
                  d_rot_x(joints[1]) @ self.T_2_3 @ rot_x(joints[2]) @
                  self.T_3_E)[:3,3]
        J[:,2] = (self.T_0_1 @ rot_z(joints[0]) @ self.T_1_2 @
                  rot_x(joints[1]) @ self.T_2_3 @ d_rot_x(joints[2]) @
                  self.T_3_E)[:3,3]
        return J
